...
def _chunk_text(text: str, chunk_size: int, overlap: int) -> list[str]:
    ...
    chunk_size and overlap are in approximate tokens (1 token ≈ 4 chars).
    """
...
def process_document(document_id: UUID) -> None:
    ...
    Creates its own DB session since the request session is closed by the time
    ...
    with Session(engine) as db_session:
        doc = document_repository.get_document(db_session, document_id)
        document_repository.update_status(db_session, document_id, "processing")

        try:
            logger.info("Processing document %s (%s)", document_id, doc.file_name)

            # Step 1: Extract text
            text = _extract_text(doc.storage_path, doc.file_type)
            ...
            document_repository.update_status(db_session, document_id, "processed")

        except Exception as e:
            logger.error("Document %s: ingestion failed — %s", document_id, e)
            document_repository.update_status(db_session, document_id, "failed")
